...
async def export_analytics(
    ...
    try:
        ...
        rows_dict = [
            {"date": datetime.now(timezone.utc).date().isoformat(), "platform": "LinkedIn", "total_applications": 5, "interviews": 1, "offers": 0},
        ]
        ...
        performance = [
            {
                "tone": row.tone,
                "total": row.total,
                "avg_response_rate": round(float(row.avg_response_rate), 4) if row.avg_response_rate else 0,
                "avg_interview_rate": round(float(row.avg_interview_rate), 4) if row.avg_interview_rate else 0,
            }
            for row in rows
        ]
