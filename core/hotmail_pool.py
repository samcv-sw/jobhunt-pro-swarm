...
"""
import json, os, random, time, logging, base64
...
def init(data_dir=None):
    ...
    if valid == 0:
        ...
        logger.info(f"[HOTMAIL-POOL] Initialized: {valid}/{len(pool)} accounts via Graph API")
...
def _get_access_token(account: dict) -> str:
    ...
    try:
        ...
        logger.warning(f"[HOTMAIL-OAUTH] Token refresh failed for {account['email']}: {resp.status_code} {resp.text[:100]}")
        return ""
...
def send_email_sync(to_email: str, msg_str: str) -> tuple:
    ...
    try:
        ...
        if msg_parsed.is_multipart():
            ...
            body_text = msg_parsed.get_payload(decode=True).decode("utf-8", errors="replace")
        ...
        graph_message = {
            "message": {
                "subject": subject,
                "body": {
                    "contentType": "HTML" if "<html" in body_text.lower() or "<div" in body_text.lower() or "<br" in body_text.lower() else "Text",
                    "content": body_text,
                },
                "toRecipients": [
        ...
        if resp.status_code == 202:
            ...
            logger.warning(f"[HOTMAIL-GRAPH] Send failed for {account['email']} to {to_email}: HTTP {resp.status_code} {error_body}")
...
def try_revive_dead_accounts(force: bool = False) -> int:
    ...
    try:
        ...
        dead_accounts = [a for a in data.get("accounts", []) if a.get("dead", False) and a.get("refresh")]
        ...
        if revived > 0:
            ...
            global _pool_cache
            _pool_cache = None
            logger.info(f"[HOTMAIL-REVIVE] Revived {revived}/{len(dead_accounts)} accounts")
