...
class ATSCracker:
    def _compute_tf_idf(
        self, word: str, resume_doc: str, job_description_doc: str
    ) -> float:
        """Mathematical algorithm to reverse engineer ATS scoring."""
        # Term Frequency
        tf = job_description_doc.lower().count(word.lower()) / max(
            len(job_description_doc.split()), 1
        )
    ...
    def inject_synonym_density(
        self, base_resume_text: str, job_description: str
    ) -> str:
        """
