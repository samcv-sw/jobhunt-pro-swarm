        ...
        with connect() as conn:
            ...
            if res:
                ...
                if _is_valid_cache_response(res_text):
                    ...
                    logger.warning("Exact cache hit contained unresolved placeholders! Invalidation triggered.")
                    try:
                        conn.execute("DELETE FROM semantic_cache WHERE prompt_hash = ?", (prompt_hash,))
            ...
            if backend == "pg":
                ...
                if res and res[1] >= similarity_threshold:
                    ...
                    if _is_valid_cache_response(res_text):
                        ...
                        logger.warning("Semantic cache similarity hit contained unresolved placeholders! Invalidation triggered.")
                        try:
                            conn.execute("DELETE FROM semantic_cache WHERE prompt_hash = ?", (p_hash,))
                        except Exception as ex:
                            logger.error(f"Failed to delete bad semantic cache entry: {ex}")
            else:
                # SQLite: Local python-based cosine similarity
                res = conn.execute("SELECT response_text, embedding, prompt_hash FROM semantic_cache WHERE embedding IS NOT NULL").fetchall()
                best_response = None
                best_similarity = -1.0
                best_hash = None
                ...
                if norm_a > 0:
                    if _NUMPY_AVAILABLE and res:
                        ...
                        try:
                            ...
                            emb_matrix = _np.array([json.loads(ej) for _, ej, _ in res], dtype=_np.float32)
                ...
                if best_response and best_similarity >= similarity_threshold:
                    if _is_valid_cache_response(best_response):
                        ...
                        logger.warning(f"Semantic cache similarity hit contained unresolved placeholders! Invalidation triggered.")
                        try:
                            conn.execute("DELETE FROM semantic_cache WHERE prompt_hash = ?", (best_hash,))
                        except Exception as ex:
                            logger.error(f"Failed to delete bad semantic cache entry: {ex}")
    except Exception as e:
...
def save_to_cache(prompt_text: str, response_text: str):
    ...
    if not _is_valid_cache_response(response_text):
        logger.warning("Attempted to save a response with unresolved placeholders to semantic cache. Skipping.")
        return
