...
"""
import requests, json, os, logging, re, urllib.parse
from datetime import datetime

logger = logging.getLogger(__name__)

SUPABASE_URL = os.environ.get("SUPABASE_URL", "https://zfvzqutgpdbsievsclhn.supabase.co")
...
class SupabaseCursor:
    ...
    def _parse_where(self, query):
        ...
        where_match = re.search(r"WHERE\s+(.+?)(?:ORDER BY|LIMIT|OFFSET|$)", query, re.IGNORECASE)
        if not where_match:
            return {}, ""
    ...
    def _parse_select(self, query, params):
        ...
        # Extract columns
        cols_match = re.search(r"SELECT\s+(.*?)\s+FROM", query, re.IGNORECASE | re.DOTALL)
        select_cols = "*"
        if cols_match:
    ...
    def _parse_insert(self, query, params):
        ...
        return "INSERT", {"method": "POST", "url": f"{SUPABASE_URL}/rest/v1/{table}", "json": row, "table": table}
    ...
    def _parse_update(self, query, params):
        ...
        return "UPDATE", {"method": "PATCH", "url": url, "json": updates, "table": table}
    ...
    def execute(self, query, params=None):
        ...
        try:
            ...
            if method == "GET":
                ...
                r = requests.post(url, headers=headers, json=info.get("json", {}), timeout=10)
            elif method == "PATCH":
                headers["Prefer"] = "return=representation"
                r = requests.patch(url, headers=headers, json=info.get("json", {}), timeout=10)
            elif method == "DELETE":
                headers["Prefer"] = "return=representation"
