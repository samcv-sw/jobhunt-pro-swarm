...
def _init_db():
    ...
    with _mutex:
        try:
            ...
            """)
            conn.execute("CREATE INDEX IF NOT EXISTS idx_email_sends_date ON email_sends(date, provider)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_email_sends_hour ON email_sends(date, hour, provider)")
            conn.commit()
            conn.close()
...
def _count_today(provider: str, account: str = "*") -> int:
    ...
    try:
        conn = sqlite3.connect(_db_path)
        if account == "*":
            row = conn.execute("SELECT COUNT(*) FROM email_sends WHERE date = ? AND provider = ?", (today, provider)).fetchone()
        else:
            row = conn.execute("SELECT COUNT(*) FROM email_sends WHERE date = ? AND provider = ? AND account = ?", (today, provider, account)).fetchone()
        conn.close()
...
def _count_hour(provider: str, account: str = "*") -> int:
    ...
    try:
        conn = sqlite3.connect(_db_path)
        if account == "*":
            row = conn.execute("SELECT COUNT(*) FROM email_sends WHERE date = ? AND hour = ? AND provider = ?", (today, hour, provider)).fetchone()
        else:
            row = conn.execute("SELECT COUNT(*) FROM email_sends WHERE date = ? AND hour = ? AND provider = ? AND account = ?", (today, hour, provider, account)).fetchone()
        conn.close()
        return row[0] if row else 0
    except Exception:
        return 0
...
def can_send_gmail(account_email: str) -> tuple[bool, str]:
    ...
    if hourly >= GMAIL_HOURLY_CAP:
        return False, f"Gmail hourly cap ({GMAIL_HOURLY_CAP}) reached for {account_email}"
    return True, "ok"
    ...
    try:
        with FileLock(LOCK_FILE, timeout=5):
            if os.path.exists(STATE_FILE):
                ...
                state = {
                    "provider_counts": {},
                    "global_counts": {"daily": 0, "hourly": 0, "last_reset_day": "", "last_reset_hour": -1},
...
def can_send_global() -> tuple[bool, str]:
    ...
    if state["failure_tracker"]["cooldown_until"] > time.time():
        ...
        return False, f"Cooldown active: {remaining}s remaining after {MAX_FAILURES_BEFORE_COOLDOWN} failures"
...
def record_failure():
    ...
    if state["failure_tracker"]["consecutive"] >= MAX_FAILURES_BEFORE_COOLDOWN:
        state["failure_tracker"]["cooldown_until"] = time.time() + (FAILURE_COOLDOWN_MINUTES * 60)
        logger.warning(f"BanShield: {MAX_FAILURES_BEFORE_COOLDOWN} consecutive failures — cooldown for {FAILURE_COOLDOWN_MINUTES}min")
    _save_state(state)
...
def smart_delay(provider: str) -> float:
    ...
    usage_ratio = state["global_counts"]["daily"] / GLOBAL_DAILY_CAP if GLOBAL_DAILY_CAP else 0
...
def get_multi_provider_cap() -> int:
    state = _get_state()
    active = len([k for k, v in state["provider_counts"].items() if v.get("daily", 0) > 0]) or 1
    tier = 1
...
def get_safe_send_window() -> dict:
    ...
    return {
        ...
        "risk_level": "low" if not is_weekend() and is_business_hours() and state["global_counts"]["daily"] < GLOBAL_DAILY_CAP * 0.5 else "moderate",
    }
