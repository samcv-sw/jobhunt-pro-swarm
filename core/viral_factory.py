...
class ViralFactory:
    ...
    async def _generate_video(self, audio_path: str, filename: str) -> str:
        ...
        cmd = [
            "ffmpeg", "-y",
            "-f", "lavfi", "-i", "color=c=black:s=1080x1920:d=1", # Vertical black background
            "-i", audio_path,
            "-filter_complex", 
    ...
    async def create_viral_video(self, script_text: str = None) -> dict:
        ...
        logger.info(f"[VIRAL FACTORY] Generating viral audio for script: {text[:30]}...")
        ...
        try:
            from core.telegram_bot import send_telegram_message_sync, TELEGRAM_CHAT_ID
            ...
            send_telegram_message_sync(f"🚀 *AUTO-VIRAL BLAST*\n\n{text}\n\n*Video rendered and ready to upload from Server:* `{video_path}`")
            logger.info("[VIRAL FACTORY] Successfully broadcasted to Telegram channels!")
