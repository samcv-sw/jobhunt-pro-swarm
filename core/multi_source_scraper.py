...
USER_AGENTS = [
    # We must pin this exactly to match curl_cffi's impersonate="chrome119" TLS fingerprint.
    # Discrepancies between TLS fingerprint and User-Agent will trigger Cloudflare blocks.
            ...
            try:
                ...
                if resp.status_code == 403 and attempt < max_retries:
                    mobile_ua = random.choice(
                        [u for u in USER_AGENTS if "Mobile" in u or "Googlebot" in u]
                    )
                    headers["User-Agent"] = mobile_ua
                    headers["Sec-Ch-Ua-Mobile"] = "?1"
                    delay = (attempt + 1) * 3
                    logger.debug(
                        f"{self.source_name}: 403 -> retry with mobile UA, attempt {attempt + 2}"
                    )
                    time.sleep(delay)
                    resp = self._session.get(url, headers=headers)
                    if resp.status_code == 200:
                        return resp
                    logger.debug(
                        f"{self.source_name}: 403 forbidden after {attempt + 1} attempts"
                    )
                    return resp
...
class BaytScraper(BaseScraper):
    ...
    def _parse_card(self, card, fallback_location: str) -> Optional[Dict]:
        ...
        location_elem = card.find("span", class_="jb-location") or card.find(
...
class NaukriScraper(BaseScraper):
    ...
    def search(self, query: str, location: str = "", limit: int = 10) -> List[Dict]:
        ...
        for loc in location_parts[:3]:
            ...
            try:
                ...
                resp = self._get(
                    url, extra_headers={"Referer": "https://www.naukrigulf.com/"}
                )
                if not resp or resp.status_code != 200:
                    # Try alternative URL format
                    if loc:
                        url = (
                            f"https://www.naukrigulf.com/jobs?q={q}&l={quote_plus(loc)}"
                        )
                        resp = self._get(url)
                        if not resp or resp.status_code != 200:
                            continue
    ...
    def _parse_card(self, card, fallback_location: str) -> Optional[Dict]:
        ...
        return self._build_job_dict(title, company, loc, job_url, salary=salary)
...
class IndeedScraper(BaseScraper):
    ...
    def _parse_card(self, card, fallback_location: str, city: str) -> Optional[Dict]:
        ...
        title_elem = (
            ...
            or card.find("a", id=re.compile(r"job_"))
            or card.find("span", title=True)
        )
        title = title_elem.get_text(strip=True) if title_elem else ""
        title = re.sub(r"\s+", " ", title).strip()
        ...
        company_elem = (
            ...
            or card.find("a", class_="companyOverviewLink")
        )
        company = company_elem.get_text(strip=True) if company_elem else ""
        ...
        location = (
            ...
            if location_elem
            else city or fallback_location
        )
        ...
        try:
            ...
            for result in soup.select("div.g")[: limit * 2]:
                try:
                    ...
                    job = self._build_job_dict(
                        title, company, loc_text, href, snippet[:200]
                    )
                    jobs.append(job)
                    if len(jobs) >= limit:
                        break
...
class LinkedInScraper(BaseScraper):
    ...
    def _parse_card(self, card, fallback_location: str) -> Optional[Dict]:
        ...
        return self._build_job_dict(
            title, company, loc, job_url, f"LinkedIn: {title} at {company}"
        )
...
class GlassdoorScraper(BaseScraper):
    ...
    def search(self, query: str, location: str = "", limit: int = 10) -> List[Dict]:
        ...
        try:
            q = quote_plus(query)
            url = (
                f"https://www.glassdoor.com/Job/jobs.htm?sc.keyword={q}&locT=C&locId=0"
            )
            ...
            resp = self._get(
                url, extra_headers={"Referer": "https://www.glassdoor.com/"}
            )
            if not resp:
                return jobs
            if resp.status_code == 403:
            ...
            if response.status_code == 200:
                ...
                for card in cards[:limit]:
                    ...
                    company = card.select_one(
                        'p[class*="company"], div[class*="company"]'
                    )
