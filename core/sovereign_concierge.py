...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] SOVEREIGN-CONCIERGE: %(message)s")
...
def assemble_fractional_pod(pod_size=4):
    ...
    if os.path.exists(DB_PATH):
        try:
            ...
            cur = conn.cursor()
            cur.execute("SELECT id, name, email FROM users ORDER BY RANDOM() LIMIT ?", (pod_size,))
            rows = cur.fetchall()
            for row in rows:
                pod.append(dict(row))
            conn.close()
    ...
    while len(pod) < pod_size:
        pod.append({
            ...
            "role": random.choice(["Architect", "Frontend", "Backend", "Data Engineer"])
        })
    return pod
...
def execute_sovereign_order():
    ...
    logger.info(f"Execution Fee: ${order['fee']} | Deadline: {order['deadline_hours']} Hours")
    ...
    logger.info(f"   -> Developer Pool: ${dev_share} (${per_dev_payout} per developer in the 3rd world)")
    logger.info("==================================================")
