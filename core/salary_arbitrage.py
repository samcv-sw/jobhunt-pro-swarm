...
class SalaryArbitrage:
    async def calculate_negotiation_leverage(self, company_name: str, city_location: str, base_offer: int) -> dict:
        """
        ...
        """
        logger.info(f"Calculating Salary Arbitrage for {company_name} in {city_location}...")
        ...
        target_counter = base_offer + macro_economics["competitor_salary_median"] - base_offer
        if macro_economics["company_stock_30d_growth"] > 0:
            target_counter += (base_offer * 0.05) # Add 5% premium for good stock performance
