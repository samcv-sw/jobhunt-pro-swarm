...
GROQ_KEYS = [k.strip() for k in (os.getenv("GROQ_API_KEY") or "").split(",") if k.strip()]
GEMINI_KEYS = [k.strip() for k in (os.getenv("GEMINI_API_KEY") or "").split(",") if k.strip()]
GITHUB_MODELS_KEYS = [k.strip() for k in (os.getenv("GITHUB_TOKEN") or "").split(",") if k.strip()]
MISTRAL_KEYS = [k.strip() for k in (os.getenv("MISTRAL_API_KEY") or "").split(",") if k.strip()]
...
class AIRouter:
    ...
    @classmethod
    async def generate_response(cls, system_prompt: str, user_prompt: str, task_type: str = "logic", max_retries: int = 3, thread_id: str = None) -> str:
        """
        ...
        if _HAS_LANGGRAPH and cls.get_pool():
            return await cls._run_langgraph(system_prompt, user_prompt, task_type, max_retries, thread_id)
        else:
            return await cls._run_native(system_prompt, user_prompt, task_type, max_retries)

    @classmethod
    async def _run_langgraph(cls, system_prompt: str, user_prompt: str, task_type: str, max_retries: int, thread_id: str) -> str:
        ...
        async def node_route(state: AgentState) -> Dict:
            ...
            if not target:
                return {"error": "All providers circuit broken", "current_provider": "none"}
        ...
        async def node_execute(state: AgentState) -> Dict:
            ...
            try:
                if provider == "mistral":
                    resp = await cls._call_mistral(state["system_prompt"], state["user_prompt"])
                elif provider == "groq":
                    resp = await cls._call_groq(state["system_prompt"], state["user_prompt"])
                elif provider == "gemini":
                    resp = await cls._call_gemini(state["system_prompt"], state["user_prompt"])
                else:
                    resp = await cls._call_github(state["system_prompt"], state["user_prompt"])
                ...
                return {"error": str(e), "current_attempt": state["current_attempt"] + 1}
        ...
        workflow.add_conditional_edges("execute", should_retry, {"route": "route", "end": END})
    ...
    @classmethod
    async def _run_native(cls, system_prompt: str, user_prompt: str, task_type: str = "logic", max_retries: int = 3) -> str:
        """Fallback to native loop if LangGraph/Postgres is unavailable."""
        providers = []
        if task_type == "sensitive":
            providers = [("mistral", cls._call_mistral, MISTRAL_KEYS), ("github", cls._call_github, GITHUB_MODELS_KEYS)]
        elif task_type == "logic":
            providers = [("groq", cls._call_groq, GROQ_KEYS), ("gemini", cls._call_gemini, GEMINI_KEYS), ("github", cls._call_github, GITHUB_MODELS_KEYS)]
        elif task_type == "context":
            providers = [("gemini", cls._call_gemini, GEMINI_KEYS), ("github", cls._call_github, GITHUB_MODELS_KEYS), ("groq", cls._call_groq, GROQ_KEYS)]
        else:
            providers = [("github", cls._call_github, GITHUB_MODELS_KEYS), ("groq", cls._call_groq, GROQ_KEYS), ("gemini", cls._call_gemini, GEMINI_KEYS)]
