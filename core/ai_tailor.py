...
CANDIDATE_PROFILE = {
    ...
    "security": [
        "Firewalls (FortiGate/Palo Alto/Checkpoint)",
        "IDS/IPS",
        "Network Security",
        "Zero Trust / ZTNA",
        "SASE",
    ...
    "cloud": [
        "AWS (VPC/Direct Connect/Route53)",
        "Azure (ExpressRoute/VNet)",
        "GCP (Cloud Interconnect/VPC)",
    ...
    "automation": [
        "Python (Netmiko/Napalm/Scrapli/Nornir)",
        "PowerShell",
        "Ansible (AWX/Tower)",
        "Terraform (IaC)",
        ...
        if result:
            try:
                ...
                if "recommendation" not in data:
                    s = data["score"]
                    data["recommendation"] = (
                        "apply" if s >= 70 else ("maybe" if s >= 50 else "skip")
                    )
                return data
...
- Values: {company_info.get("values", "N/A")}"""
        ...
        cv_subset = []
        cv_subset.append(
            f"- Title: {CANDIDATE_PROFILE['title']}, {CANDIDATE_PROFILE['years']}+ years experience"
        )
        ...
        if any(
            ...
            cv_subset.append(
                f"- Core expertise: {', '.join(CANDIDATE_PROFILE['core_skills'])}"
            )
            cv_subset.append(
                f"- Protocols: {', '.join(CANDIDATE_PROFILE['protocols'])}"
            )
        ...
        if any(
            ...
            cv_subset.append(
                f"- Monitoring/Observability: {', '.join(CANDIDATE_PROFILE['monitoring'])}"
            )

        cv_subset.append(f"- Certifications: {', '.join(CANDIDATE_PROFILE['certs'])}")
        cv_subset.append(
            f"- Key achievements: {'; '.join(CANDIDATE_PROFILE['highlights'][:3])}"
        )  # Top 3 achievements
...
9. Sign as: Sam Salameh, {config.CANDIDATE_EMAIL}, {config.CANDIDATE_PHONE}"""
    ...
    async def generate_ats_white_text(
...
Format: Return ONLY a raw comma-separated list of the 30-40 most critical keywords and phrases, separated by commas. No formatting, no bullet points, no introduction. Just the raw keywords."""
    ...
    async def optimize_semantic_density(
        self, company: str, title: str, description: str, current_cv_text: str
    ) -> str:
        """
...
6. Be specific, not generic — avoid "results-driven professional" type phrases"""
    ...
    async def generate_icebreaker(
        self, company: str, title: str, description: str = "", company_info: dict = None
    ) -> str:
...
Return ONLY the icebreaker text, nothing else."""
        ...
        return (
            result.strip()
            if result
            else f"I am particularly drawn to the {title} opportunity at {company} as it aligns with my extensive network engineering background."
        )
...
Only include information you are confident about. Use empty string for unknown fields."""
    ...
    async def build_knowledge_graph_intro(
        self, recruiter_name: str, company: str
    ) -> str:
        """
        [THE ABSOLUTE MAXIMUM]
...
Return ONLY the introductory sentence."""
        ...
        return (
            result
            or f"Hi {recruiter_name}, noticed we share a mutual passion for enterprise networking and I've been following {company}'s recent infrastructure growth."
        )
...
- Company-specific questions (why this company, cultural fit)"""
    ...
    async def generate_thank_you(
        self, company: str, title: str, interview_date: str = ""
    ) -> str:
        """Generate a post-interview thank you email."""
        date_context = (
            f" after the interview on {interview_date}" if interview_date else ""
        )
    ...
    async def _call_ai(
        ...
        try:
            ...
            if cached_res:
                logger.info(
                    f"[$0 PGVECTOR SEMANTIC CACHE HIT] Saved {max_tokens} tokens."
                )
                return cached_res
        except Exception as e:
        ...
        def _is_valid_ai_response(text: str) -> bool:
            ...
            has_unresolved = any(
                placeholder.lower() in text.lower()
        ...
        for attempt in range(1, 4):
            ...
            if self.llm_pool:
                try:
                    ...
                    logger.warning(
                        f"Rotating LLM Provider Pool failed on attempt {attempt}: {e}. Falling back to legacy Groq/Gemini..."
                    )
                ...
                for model in self.MODELS:
                    try:
                        result = await self._call_groq(
                            current_prompt, model, max_tokens, temperature
                        )
                        if result:
                            break
                    except Exception as e:
        ...
        logger.error(
            "[Self-Healing AI] All AI providers failed or generated invalid responses after 3 attempts."
        )
        return None

    async def _exponential_backoff_with_jitter(
        self, attempt: int, base_delay: float = 2.0, max_delay: float = 120.0
    ) -> float:
        """
    ...
    async def _call_groq(
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are a professional career advisor and cover letter writer. Always follow the user's formatting instructions precisely. When asked for JSON, return ONLY valid JSON without markdown code fences.",
                },
    ...
    def _cosine_similarity_score(title: str, description: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                "missing_skills": [],
                "experience_fit": "engineer" in job_text
                or "manager" in job_text
                or "architect" in job_text
                or "director" in job_text
                or "vp" in job_text
                or "cto" in job_text,
                "domain_fit": any(
                    kw in job_text
                ...
                if score >= 65
                else ("maybe" if score >= 45 else "skip"),
                "reason": f"Vector Cosine Similarity: {cos_sim:.2f}",
            }
        except Exception as e:
            logger.warning(
                f"Cosine similarity failed, falling back to keyword math: {e}"
            )
    ...
    def _fallback_score(title: str, description: str) -> dict:
        ...
        return {
            ...
            "skills_match": matching[:15],
            "missing_skills": [],
            "experience_fit": "engineer" in text
