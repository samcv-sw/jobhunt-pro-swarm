...
class ViralSquadManager:
    ...
    def create_squad(self, owner_id: str) -> str:
        ...
        try:
            ...
            # Update user
            self.db.execute(
                "UPDATE users SET squad_id=? WHERE user_id=?",
                (squad_id, owner_id)
            )
            self.db.commit()
            return squad_id
    ...
    def join_squad(self, user_id: str, squad_id: str) -> bool:
        ...
        try:
            ...
            squad = self.db.execute("SELECT member_count FROM job_squads WHERE squad_id=?", (squad_id,)).fetchone()
            if not squad:
                return False
            ...
            if is_complete:
                logger.info(f"Squad {squad_id} is now complete! Video Pitcher Unlocked for members.")
    ...
    def check_video_pitcher_access(self, user_id: str) -> bool:
        ...
        try:
            user = self.db.execute("SELECT squad_id, subscription_status FROM users WHERE user_id=?", (user_id,)).fetchone()
            if not user:
                return False
            ...
            if user["squad_id"]:
                squad = self.db.execute("SELECT is_complete FROM job_squads WHERE squad_id=?", (user["squad_id"],)).fetchone()
    ...
    def generate_invite_link(self, squad_id: str, base_url: str = "https://t.me/JobHuntProBot") -> str:
