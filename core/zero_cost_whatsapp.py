...
class ZeroCostWhatsAppAutomator:
    ...
    async def send_whatsapp_message(self, phone_number: str, candidate_name: str, company: str, position: str) -> dict:
        """
        Sends a free WhatsApp message using Meta's REST API.
        """
        if not self.access_token or not self.phone_id:
            logger.warning("WhatsApp API credentials missing. Simulating Cloud API request.")
        ...
        try:
            logger.info(f"Preparing to send Cloud WhatsApp message to {formatted_phone}...")
