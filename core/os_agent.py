...
class OSAgent:
    ...
    async def start_browser(self, headless: bool = True, proxy: Optional[str] = None, session_id: Optional[str] = None):
        ...
        if session_id:
            if session_id not in self._sticky_sessions:
                self._sticky_sessions[session_id] = await AsyncCamoufox(**args).__aenter__()
    ...
    async def human_mouse_move(self, target_x: int, target_y: int):
        ...
        try:
            ...
            await HumanMouse.simulate_mouse_movement(self.page, start_x, start_y, target_x, target_y)
        except Exception:
    ...
    async def physical_click_element(self, selector: str):
        ...
        try:
            ...
            await self.page.mouse.click(center_x, center_y, delay=random.randint(50, 150))
