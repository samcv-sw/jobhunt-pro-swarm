...
class JobKnowledgeGraph:
    ...
    def _check_dependencies(self):
        try:
            ...
            logger.warning(
                "networkx not installed. Falling back to simple dict-based graph."
            )
            self.G = None
            self.has_nx = False
    ...
    def add_edge(
        ...
        self.edges[source_id].append(
            {"target": target_id, "rel": relationship, "weight": weight}
        )
        ...
        logger.debug(
            f"Added Graph Edge: [{source_id}] -({relationship})-> [{target_id}]"
        )
    ...
    def find_backdoor_routes(self, target_company_id: str) -> list:
        ...
        # Find all recruiters
        recruiters = [
            n for n, attr in self.G.nodes(data=True) if attr.get("type") == "Recruiter"
        ]
    ...
    def ingest_job_listing(self, job_data: dict):
        ...
        company_id = (
            f"comp_{job_data.get('company', 'unknown').replace(' ', '_').lower()}"
        )
