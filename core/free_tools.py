...
def init(data_dir: Optional[str] = None):
    ...
    if not USAGE_FILE.exists():
        with open(USAGE_FILE, 'w') as f:
            json.dump({"ats_checks": 0, "cover_letters": 0, "salary_calcs": 0, "total": 0}, f)


# ── ATS Resume Checker ───────────────────────────────────────

ATS_KEYWORDS = {
    "networking": ["tcp/ip", "ospf", "bgp", "vlan", "mpls", "wan", "lan", "dhcp", "dns", "vpn",
                   "routing", "switching", "firewall", "subnet", "nat", "qos", "stp", "eigrp"],
    "cloud": ["aws", "azure", "gcp", "kubernetes", "docker", "terraform", "ansible", "jenkins",
              "devops", "ci/cd", "serverless", "ec2", "s3", "lambda"],
    "programming": ["python", "java", "javascript", "typescript", "go", "rust", "c++", "sql",
                    "react", "node", "api", "rest", "git", "agile"],
    "security": ["cybersecurity", "firewall", "encryption", "siem", "soc", "penetration testing",
                 "iso 27001", "nist", "gdpr", "hips"],
    ...
    "certifications": ["ccna", "ccnp", "ccie", "comptia", "pmp", "aws certified", "azure certified",
                       "cissp", "cism", "itil", "scrum master"],
}
...
def check_ats_resume(resume_text: str, target_role: str = "general") -> Dict:
    ...
    sections = {
        ...
        "summary": bool(re.search(r'(summary|profile|objective|about\s+me)', text_lower)),
        ...
        "skills": bool(re.search(r'(skills|competencies|expertise|technologies)', text_lower)),
    }
    results["sections"] = {k: "✅ Found" if v else "❌ Missing" for k, v in sections.items()}
    ...
    for category, keywords in ATS_KEYWORDS.items():
        ...
        kw_breakdown[category] = {"found": len(found), "total": len(keywords), "keywords": found[:5]}
    ...
    if word_count > 2000:
        results["format_issues"].append("Resume too long (over 2,000 words — aim for 1-2 pages)")
    if "\t" in resume_text:
        results["format_issues"].append("Contains tabs — use spaces for ATS compatibility")
    ...
    # 4. Calculate scores
    section_score = sum(1 for v in sections.values() if v) / len(sections) * 25  # 25% weight
    kw_density = min(total_kw_matches / 20, 1.0) * 35  # 35% weight
    length_score = (1.0 if 300 <= word_count <= 1500 else 0.5) * 15  # 15% weight
    format_score = max(0, (1.0 - len(results["format_issues"]) * 0.1)) * 25  # 25% weight
    ...
    if results["overall_score"] < 40:
        results["recommendations"].append("⚠️ Your resume needs significant improvement for ATS systems")
    elif results["overall_score"] < 70:
        results["recommendations"].append("📈 Good start — focus on adding more industry keywords")
    else:
        results["recommendations"].append("✅ Strong ATS compatibility — keep optimizing!")
...
SALARY_DATA = {
    "software engineer": {"us": (80000, 180000), "lebanon": (12000, 48000), "uae": (60000, 150000), "remote": (50000, 200000)},
    "network engineer": {"us": (70000, 150000), "lebanon": (12000, 40000), "uae": (50000, 120000), "remote": (40000, 160000)},
    "data scientist": {"us": (90000, 200000), "lebanon": (15000, 50000), "uae": (70000, 160000), "remote": (60000, 220000)},
    "project manager": {"us": (70000, 160000), "lebanon": (15000, 45000), "uae": (60000, 140000), "remote": (50000, 170000)},
    "cybersecurity analyst": {"us": (75000, 170000), "lebanon": (15000, 45000), "uae": (65000, 150000), "remote": (55000, 180000)},
    "devops engineer": {"us": (85000, 180000), "lebanon": (18000, 55000), "uae": (70000, 160000), "remote": (60000, 200000)},
    "cloud architect": {"us": (100000, 220000), "lebanon": (20000, 60000), "uae": (80000, 180000), "remote": (70000, 240000)},
    "hr manager": {"us": (60000, 130000), "lebanon": (12000, 36000), "uae": (50000, 120000), "remote": (40000, 140000)},
    "product manager": {"us": (80000, 180000), "lebanon": (18000, 55000), "uae": (70000, 160000), "remote": (60000, 200000)},
    "ux designer": {"us": (65000, 150000), "lebanon": (10000, 35000), "uae": (50000, 130000), "remote": (45000, 160000)},
    "default": {"us": (50000, 120000), "lebanon": (8000, 30000), "uae": (40000, 100000), "remote": (35000, 140000)},
}


def calculate_salary(job_title: str, location: str = "us", experience_years: int = 5) -> Dict:
