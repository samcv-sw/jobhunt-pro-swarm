...
class GhostApplicant:
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                if "greenhouse.io" in url or "application_form" in content.lower():
                    ...
                    logger.info(
                        "[GHOST] Generic/Unknown portal. Attempting universal fallback fill."
                    )
    ...
    async def _fill_greenhouse(self, page, profile: dict, cv_path: str) -> bool:
        ...
        try:
            # First Name
            if (
                await page.locator("input[name='job_application[first_name]']").count()
                > 0
            ):
                await page.fill(
            ...
            # Last Name
            if (
                await page.locator("input[name='job_application[last_name]']").count()
                > 0
            ):
            ...
            submit_btn = page.locator(
                "input[type='submit'], button[type='submit'], button[id='submit_app'], button:has-text('Submit')"
            )
    ...
    async def _fill_lever(self, page, profile: dict, cv_path: str) -> bool:
        ...
        try:
            ...
            # Submit application form
            submit_btn = page.locator(
                "input[type='submit'], button[type='submit']:not([aria-label='Close']), button:has-text('Submit')"
            )
            if await submit_btn.count() > 0:
                await submit_btn.first.click()
    ...
    async def _fill_generic(self, page, profile: dict, cv_path: str) -> bool:
        ...
        try:
            ...
            # Submit application form
            submit_btn = page.locator(
                "input[type='submit'], button[type='submit'], button:has-text('Send'), button:has-text('Apply'), button:has-text('Submit')"
            )
            if await submit_btn.count() > 0:
                await submit_btn.first.click()
                await page.wait_for_timeout(2000)
