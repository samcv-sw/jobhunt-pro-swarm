...
class GhostApplicant:
        ...
        async with sem:
            ...
            try:
                ...
                async with async_playwright() as p:
                    ...
                    # Setup image and tracker blocking using Playwright routing to save bandwidth
                    ...
                    logger.info("[GhostApplicant] Resource blocking configured (images, trackers).")
    ...
    async def _fill_field(self, page_or_frame, selector: str, value: str):
        ...
        if await loc.count() > 0:
            first_loc = loc.first
            await first_loc.scroll_into_view_if_needed()

            # Simulate mouse movement before focusing/typing
            page = page_or_frame if hasattr(page_or_frame, "mouse") else getattr(page_or_frame, "page", None)
            if page and hasattr(page, "mouse"):
                try:
                    box = await first_loc.bounding_box()
                    if box:
                        start_x = random.randint(0, 100)
                        start_y = random.randint(0, 100)
                        dest_x = int(box["x"] + box["width"] / 2)
                        dest_y = int(box["y"] + box["height"] / 2)
                        await HumanMouse.simulate_mouse_movement(page, start_x, start_y, dest_x, dest_y)
                except Exception as e:
                    logger.debug(f"Field hover simulation failed: {e}")

            await self._type_humanlike(first_loc, value)
    ...
    async def _upload_file(self, page_or_frame, selector: str, file_path: str):
        ...
        if await loc.count() > 0:
            ...
            # Simulate mouse movement before uploading
            page = page_or_frame if hasattr(page_or_frame, "mouse") else getattr(page_or_frame, "page", None)
            if page and hasattr(page, "mouse"):
                try:
                    box = await first_loc.bounding_box()
                    ...
                    logger.debug(f"File upload hover simulation failed: {e}")

            await first_loc.set_input_files(file_path)
        ...
        if await submit_btn.count() > 0:
            ...
            if box:
                ...
                try:
                    ...
                    logger.debug(f"Mouse simulation failed: {mouse_err}")
                    await page.mouse.move(dest_x, dest_y, steps=10)
            ...
            logger.warning("[GHOST] Submit button not found — returning partial success (autofill + resource blocking succeeded)")
            return True  # Autofill and resource blocking worked; submit button absence is non-fatal
