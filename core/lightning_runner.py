...
async def run_campaign_lightning(campaign_id: str, company_limit: int = 3) -> dict:
    ...
    try:
        ...
        tenant_row = (
            dict(tenant_row)
            if tenant_row and not isinstance(tenant_row, dict)
            else tenant_row
        )
        ...
        if profile_id:
            ...
            profile = (
                dict(profile) if profile and not isinstance(profile, dict) else profile
            )
        ...
        tenant_email = (
            tenant_row.get("email", "samatou683@gmail.com")
            if tenant_row
            else "samatou683@gmail.com"
        )
        ...
        logger.info(
            f"[Lightning v3] Campaign {campaign_id}: {len(companies)} companies ({role_type}) - {tenant_name}"
        )
            ...
            for company in companies:
                ...
                if not company_email or "@" not in company_email:
                    ...
                    companies_processed.append(
                        {"company": company_name, "status": "invalid_email"}
                    )
                    continue
                ...
                can_apply, reason = anti_ban.can_apply_to_company(
                    company_name, user_id=tenant_id
                )
                if not can_apply:
                    failed += 1
                    companies_processed.append(
...
<p style="margin-top:20px">Best regards,<br><strong>{tenant_name}</strong><br>📧 {tenant_email}</p>
            ...
            for detail in result.get("details", []):
                ...
                if "sent" in status:
                    try:
                        ...
                        logger.error(
                            f"[Lightning] Failed to record successful application: {ex}"
                        )
                else:
                    ...
                    try:
                        ...
                        logger.error(
                            f"[Lightning] Failed to record failed application: {ex}"
                        )
            ...
            for c in companies:
                companies_processed.append(
                    {"company": c["company_name"], "status": "logged"}
                )
        ...
        if has_more:
            ...
            logger.info(
                f"[Lightning] Campaign {campaign_id} updated: sent={new_sent}/{new_total}, status remains pending"
            )
        else:
            conn.execute(
            ...
            logger.info(
                f"[Lightning] Campaign {campaign_id} COMPLETED: sent={new_sent}/{new_total}"
            )
