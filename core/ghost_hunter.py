...
class GhostHunter:
    ...
    def hunt_for_user(self, user_id: str, job_title: str, location: str, max_jobs: int = 5):
        ...
        logger.info(f"[DATASET-FETCHER] Found {len(urls)} data URLs. Extracting features...")
            ...
            with Camoufox(headless=True) as browser:
                ...
                for url in urls:
                    try:
                        ...
                        existing = conn.execute("SELECT 1 FROM jobs WHERE user_id = ? AND url = ?", (user_id, url)).fetchone()
                        if existing:
                            conn.close()
                            continue
    ...
    def run_all_users(self):
        ...
        try:
            conn = get_db()
            users = conn.execute("SELECT user_id, cv_text FROM users WHERE cv_text IS NOT NULL").fetchall()
            conn.close()
