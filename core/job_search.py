...
def get_dynamic_headers() -> dict[str, str]:
    ...
    headers = {
        ...
        "Accept-Language": random.choice(
            ["en-US,en;q=0.9", "en-GB,en;q=0.8,en-US;q=0.6", "en-US,en;q=0.5"]
        ),
        "DNT": "1",
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if e.response.status_code == 429:
                        ...
                        retry_after = int(
                            e.response.headers.get(
                                "Retry-After", base_delay**attempt * 2
                            )
                        )
                        logger.warning(
                            f"[{func.__name__}] HTTP 429 Rate Limit Hit. Sleeping {retry_after}s..."
                        )
                    ...
                    if attempt == max_retries:
                        logger.error(
                            f"[{func.__name__}] Terminal HTTP error: {e.response.status_code}"
                        )
                        return []

                    delay = base_delay**attempt + random.uniform(0, 1)
                    logger.warning(
                        f"[{func.__name__}] HTTP error {e.response.status_code}. Retrying in {delay:.2f}s..."
                    )
                    ...
                    delay = base_delay**attempt + random.uniform(0, 1)
                    logger.warning(
                        f"[{func.__name__}] Network error: {e}. Retrying in {delay:.2f}s..."
                    )
                    await asyncio.sleep(delay)

                except Exception as e:
                    logger.error(
                        f"[{func.__name__}] Unexpected Critical Error: {e}",
                        exc_info=True,
                    )
                    return []
            return []
...
class DarkWebScraper(BaseJobScraper):
    ...
    async def search(
        ...
        resp = await _shared_client.get(
            f"https://remoteok.com/api?tag={search_q}", headers=get_dynamic_headers()
        )
        resp.raise_for_status()
...
class ArbeitnowScraper(BaseJobScraper):
    ...
    async def search(
        ...
        for item in data:
            ...
            if (
                location
                and location.lower() not in loc.lower()
                and not item.get("remote")
            ):
                continue
...
class EnterpriseJobSearch:
    ...
    async def _execute_scraper_safely(
        ...
        async with self.semaphore:
            try:
                ...
                logger.error(
                    f"[EnterpriseEngine] Isolation caught failure in {scraper.__class__.__name__}: {e}",
                    exc_info=True,
                )
                return []
    ...
    async def execute_parallel_search(
        ...
        logger.info(
            f"Enterprise Engine: Yielded {len(unique_jobs)} highly-qualified leads.",
            extra={"query": query, "location": location, "results": len(unique_jobs)},
        )
...
class MultiSourceSearch:
    ...
    def _init_scrapers(self):
        ...
        try:
            ...
            logger.info(
                f"MultiSourceSearch: initialized {len(self._scrapers)} scrapers"
            )
        except ImportError as e:
            logger.warning(
                f"MultiSourceSearch: multi_source_scraper not available: {e}"
            )
            self._scrapers = []
            ...
            for title in selected_titles:
                ...
                try:
                    jobs = self.search_all_sources(
                        query=title, location=location, limit=30
                    )
                    for j in jobs:
                        jid = j.get("job_id", "")
                        if jid and jid not in seen_ids:
