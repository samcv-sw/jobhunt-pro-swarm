...
def analyze_and_respond(request):
    logger.info(f"🐕‍🦺 [CERBERUS] Incoming Request from {request['client']}: \"{request['query']}\"")
    logger.info("🐕‍🦺 [CERBERUS] Analyzing intent via NLP heuristics...")
    
    if request['intent'] == "CUSTOM_UPSELL":
    ...
    elif request['intent'] in ["IP_THEFT_CLONE", "IP_THEFT_SOURCE"]:
