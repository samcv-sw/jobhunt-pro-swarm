...
def analyze_and_respond(request: dict) -> bool:
    ...
    try:
        ...
        if intent == "CUSTOM_UPSELL":
            ...
            logger.info(
                f"🐕‍🦺 [CERBERUS] Action: Generated automated SOW. Quoting ${quote}."
            )
            logger.info(
                f"🐕‍🦺 [CERBERUS] Payment Link Sent: https://pay.leviathan.io/custom-project-{quote}"
            )
            logger.info(
                "🐕‍🦺 [CERBERUS] Once paid, the project will be auto-sliced to the Swarm for $0 cost."
            )

        elif intent in ["IP_THEFT_CLONE", "IP_THEFT_SOURCE"]:
            logger.warning(
                "🐕‍🦺 [CERBERUS] 🚨 WARNING: INTELLECTUAL PROPERTY THEFT DETECTED 🚨"
            )
            logger.warning(
                "🐕‍🦺 [CERBERUS] Intent: Attempting to steal or clone Monopoly Architecture."
            )
            ...
            logger.warning(
                f"🐕‍🦺 [CERBERUS] Unknown intent '{intent}' from {client} — flagging for review."
            )
