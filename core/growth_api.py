...
async def blaster_upload(request: Request):
    ...
    try:
        ...
        from core.email_harvester import load_from_csv as lc, load_from_json as lj, load_from_txt as lt
        ...
        return {"status": "ok", "count": len(recipients), "path": str(dest), "sample": recipients[:3]}
...
async def blog_list(page: int = Query(1, ge=1), limit: int = Query(10, le=50)):
    ...
    try:
        ...
        return {
            "status": "ok",
            "page": page,
            "posts": [{"title": p["title"], "slug": p["slug"], "meta": p.get("meta_description", ""),
                       "published": p.get("published_at", ""), "keyword": p.get("primary_keyword", "")}
                      for p in posts]
        }
    except Exception as e:
        return {"status": "error", "error": str(e)}
...
async def free_ats_check(request: Request):
    ...
    try:
        ...
        if not resume_text or len(resume_text) < 50:
            return {"status": "error", "error": "Please provide resume text (min 50 characters)"}
...
async def recipients_build(request: Request):
    try:
        ...
        recips = build_list(audience, location=location, job_title=job_title, count=count)
    ...
    try:
        ...
        if DATA_DIR:
            for f in sorted(DATA_DIR.glob("blast_*.json")):
                ...
                files.append({"name": f.name, "count": len(data), "size_kb": size // 1024})
...
async def viral_share_card(tool: str, score: int = 0, low: int = 50, high: int = 120, job: str = "", location: str = ""):
    try:
        from core.viral_engine import get_share_card
        data = {"score": score, "low": low, "high": high, "job": job, "location": location}
...
async def viral_ph_assets():
    try:
        ...
        return {"status": "ok", "assets": get_ph_assets(), "checklist": get_ph_checklist()}
    except Exception as e:
        return {"status": "error", "error": str(e)}
...
async def viral_referral_tiers():
    try:
        ...
        return {"status": "ok", "tiers": get_referral_tiers(), "share_text": get_share_text()}
    except Exception as e:
        return {"status": "error", "error": str(e)}
...
async def graph_blaster_send(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        def _do_blast():
            ...
            gs_init()
            result = send_bulk(recipients, subject=subject, body_html=body,
                              campaign_name=campaign, max_sends=max_sends)
            logger.info(f"Background blast done: sent={result.get('sent',0)} failed={result.get('failed',0)}")

        background_tasks.add_task(_do_blast)

        return {"status": "ok", "message": f"Blast queued: {total} emails",
                "campaign": campaign, "total_queue": total}
    except Exception as e:
...
async def graph_blaster_test(request: Request):
    ...
    try:
        data = await request.json()
        to_email = data.get('to', 'samatou683@gmail.com')
...
async def blast_queue(request: Request):
    ...
    try:
        data = await request.json()
        from core.blast_queue import init as bq_init, enqueue, get_status as bq_status
        bq_init()
        job = enqueue(
