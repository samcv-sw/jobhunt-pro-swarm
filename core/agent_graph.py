...
try:
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--user", "langgraph", "langchain-openai", "langchain-groq", "langchain-core"])
    import langgraph
...
def cheat_sheet_generator_node(state: AgentState):
    ...
    if prep_html.startswith("```html"): prep_html = prep_html[7:]
    if prep_html.endswith("```"): prep_html = prep_html[:-3]
...
def run_interview_prep_graph(company: str, job_title: str, cover_letter: str, cv_text: str) -> str:
    initial_state = {
        "company": company,
        "job_title": job_title,
        "job_description": "", 
