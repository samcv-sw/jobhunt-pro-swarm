...
async def _process_growth_task(task_type: str, payload: dict[str, Any], task_id: int) -> None:
    ...
    Args:
        task_type: The type of the growth task.
        payload: Metadata payload for the task.
...
async def _process_campaign_task(task_id: int, payload: dict[str, Any]) -> None:
    ...
    Args:
        task_id: The task ID.
        payload: Task payload containing campaign_id.
    """
...
async def _process_cron_tick_task(task_id: int) -> None:
    """
    Process the cron tick task including sync engine and dataset fetching.
    ...
    while True:
        try:
            ...
            if task_type.startswith("mega_task_"):
                ...
                await _process_campaign_task(task_id, payload)

            elif task_type.startswith("growth_"):
                await _process_growth_task(task_type, payload, task_id)

            elif task_type == "cron_tick":
                await _process_cron_tick_task(task_id)
