...
class CompanyResearch:
    ...
    def research(self, company_name):
        ...
        try:
            with DDGS() as ddgs:
                try:
                    results = ddgs.text(
                        f"{company_name} company overview about us", max_results=5
                    )
                    info["info"] = " | ".join(
                        [r.get("body", "")[:200] for r in results if r.get("body")]
                    )
                ...
                try:
                    results = ddgs.text(
                        f"{company_name} recent news 2024 2025", max_results=5
                    )
                    info["news"] = " | ".join(
                        [r.get("body", "")[:200] for r in results if r.get("body")]
                    )
                ...
                try:
                    results = ddgs.text(
                        f"{company_name} network engineer salary range compensation",
                        max_results=3,
                    )
                    info["salary"] = " | ".join(
                        [r.get("body", "")[:200] for r in results if r.get("body")]
                ...
                try:
                    results = ddgs.text(
                        f"{company_name} glassdoor reviews rating", max_results=3
                    )
                    info["glassdoor"] = " | ".join(
                        [r.get("body", "")[:200] for r in results if r.get("body")]
                    )
                ...
                try:
                    results = ddgs.text(
                        f"{company_name} linkedin company page careers", max_results=3
                    )
                    for r in results:
                        url = r.get("href", "")
                ...
                try:
                    results = ddgs.text(
                        f"{company_name} careers page jobs apply", max_results=3
                    )
                    for r in results:
                        url = r.get("href", "")
                    ...
                    logger.warning(
                        f"Career page research failed for {company_name}: {e}"
                    )
        except Exception as e:
    ...
    def get_icebreakers(self, company_info):
        ...
        if company_info.get("info"):
            breakers.append(
                f"I have been following {company}'s recent developments and am impressed by your growth trajectory."
            )
        ...
        if company_info.get("culture"):
            breakers.append(
                f"{company}'s commitment to excellence aligns perfectly with my professional values."
            )
        return breakers[:3]
