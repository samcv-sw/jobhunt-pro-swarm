...
class CompanyResearch:
    ...
    def research(self, company_name):
        ...
        try:
            results = self.ddgs.text(f"{company_name} company overview about us", max_results=5)
            info['info'] = ' | '.join([r.get('body', '')[:200] for r in results if r.get('body')])
        except Exception as e:
            logger.warning(f"Research failed for {company_name}: {e}")

        try:
            results = self.ddgs.text(f"{company_name} recent news 2024 2025", max_results=5)
            info['news'] = ' | '.join([r.get('body', '')[:200] for r in results if r.get('body')])
        except Exception as e:
            logger.warning(f"News research failed for {company_name}: {e}")

        try:
            results = self.ddgs.text(f"{company_name} company culture work environment employee reviews", max_results=5)
        ...
        try:
            results = self.ddgs.text(f"{company_name} network engineer salary range compensation", max_results=3)
            info['salary'] = ' | '.join([r.get('body', '')[:200] for r in results if r.get('body')])
        except Exception as e:
            logger.warning(f"Salary research failed for {company_name}: {e}")

        try:
            results = self.ddgs.text(f"{company_name} glassdoor reviews rating", max_results=3)
        ...
        try:
            results = self.ddgs.text(f"{company_name} linkedin company page careers", max_results=3)
            for r in results:
                url = r.get('href', '')
        ...
        try:
            results = self.ddgs.text(f"{company_name} careers page jobs apply", max_results=3)
            for r in results:
                url = r.get('href', '')
