...
(function() {
  ...
  function applyLang(lang) {
    ...
    // Translate text nodes (walk through small text nodes)
    if (lang === 'ar') {
      ...
      for (const node of nodes) {
        ...
        for (const [en, ar] of Object.entries(DICT)) {
          if (text === en || text.startsWith(en + ' ') || text.endsWith(' ' + en)) {
            ...
            // Only replace if parent allows (not inside script/style)
  ...
  btn.onclick = () => {
    ...
    // Reload to get server-side language
    if (current === 'ar') return; // already switching to en via URL
    const url = new URL(window.location);
