...
def new_campaign_page(request: Request):
    ...
    content = render_template(
        ...
        user=user,
        user_id=user_id,
        active_page="new-campaign",
        profiles=profiles,
        pricing=pricing_data,
        balance=user["wallet_balance"],
    )
    return HTMLResponse(
        _build_dashboard_shell(user, user_id, content, "New Campaign", "new-campaign")
    )
...
async def delete_cv_profile(request: Request):
    ...
    try:
        ...
        conn = get_db()
        row = conn.execute(
            "SELECT id FROM cv_profiles WHERE id=? AND user_id=?", (profile_id, user_id)
        ).fetchone()
        if not row:
            conn.close()
            return JSONResponse(
                {"success": False, "message": "Profile not found or unauthorized"}
            )

        conn.execute(
            "DELETE FROM cv_profiles WHERE id=? AND user_id=?", (profile_id, user_id)
        )
        conn.commit()
        conn.close()
...
def create_campaign(
    ...
    bouquet: str = Form(""),
    bouquet_names: str = Form(""),
    engine_type: str = Form("cloud"),
):
    user_id = get_verified_user_id(request)
    if not user_id:
        return RedirectResponse("/login", status_code=303)
    ...
    try:
        ...
        conn.execute(
            """INSERT INTO campaigns (campaign_id, user_id, order_id, profile_id, total_companies, bouquets, engine_type)
                        VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (
                campaign_id,
                user_id,
        ...
        conn.execute(
            ...
            (
                ...
                -total_price,
                new_balance,
                f"Campaign: {company_count} companies",
            ),
        )
        ...
        conn.close()
        return HTMLResponse(
            "Internal Server Error: Could not process transaction.", status_code=500
        )
    finally:
        conn.close()
...
def campaign_detail(request: Request, campaign_id: str):
    ...
    content_html = render_template(
        "campaign_detail.html", request=request, campaign=campaign, emails=emails
    )
    return HTMLResponse(
        _build_dashboard_shell(
            ...
            user_id,
            content_html,
            f"Campaign {campaign.get('campaign_name', 'Detail')}",
            "new-campaign",
        )
    )
...
def campaign_war_room(request: Request, campaign_id: str):
    ...
    if weapon_count >= 14:
        weapon_badge = (
            '<span class="wb-god">💀 GOD MODE V4 — 15/15 WEAPONS ACTIVE</span>'
        )
        ...
        weapon_badge = (
            f'<span class="wb-starter">⚡ {weapon_count} WEAPON(S) ACTIVE</span>'
        )
    else:
    ...
    failed = sum(
        1
        for e in emails
        if e.get("status") in ("pending", "brevo_failed", "gmail_failed")
    )
    ...
    for e in emails:
        ...
        if cs:
            heat, heat_cls = (
                ("FIRE HOT", "hot")
                if cs >= 80
                else (("GOOD", "good") if cs >= 70 else ("COLD", "cold"))
            )
            comp_rows.append(
                {
    ...
    linkedin_rows = [
        {
            "company": e.get("company_name", "?"),
            "job_title": e.get("job_title", ""),
            "message": (e.get("linkedin_message") or "")
            .replace("&", "&amp;")
            .replace("<", "&lt;")
            .replace(">", "&gt;"),
        }
        for e in emails
        if (e.get("linkedin_message") or "").strip()
    ]
    ...
    interview_rows = [
        {
            "company": e.get("company_name", "?"),
            "job_title": e.get("job_title", ""),
            "prep": (e.get("interview_prep") or "")
            .replace("&", "&amp;")
            .replace("<", "&lt;")
            .replace(">", "&gt;"),
        }
        for e in emails
        if (e.get("interview_prep") or "").strip()
    ]
        ...
        cover_rows.append(
            {
                ...
                "id": e.get("id"),
                "status": status,
                "status_cls": "sent" if status == "SENT" else "failed",
                "html": ch.replace("&", "&amp;")
                .replace("<", "&lt;")
                .replace(">", "&gt;")
                if ch
        ...
        actions = [
            {
                "sev": "crit",
                "label": "CRITICAL",
                "text": "Send follow-ups to non-responsive companies (Day 3/7/14)",
            },
            {
                "sev": "high",
                "label": "HIGH",
                "text": "Send LinkedIn connection requests (open LinkedIn Station below)",
            },
            {
                "sev": "high",
                "label": "HIGH",
                "text": "Practice interview questions per company (open Armory below)",
            },
            {
                "sev": "med",
                "label": "MED",
                "text": "Launch a new campaign targeting different locations/roles",
            },
        ]
    elif st == "running":
        actions = [
            {
                "sev": "info",
                "label": "INFO",
                "text": "Campaign is actively sending — this page auto-refreshes",
            }
        ]
    elif st == "failed":
        actions = [
            {
                "sev": "crit",
                "label": "CRITICAL",
                "text": "Campaign failed — check error details and retry",
            }
        ]
    else:
        actions = [
            {
                "sev": "info",
                "label": "INFO",
                "text": "Campaign pending — refresh when started",
            }
        ]
    ...
    return HTMLResponse(
        _build_dashboard_shell(user_dict, user_id, content, "⚔️ War Room", "war-room")
    )
...
async def api_generate_interview_prep(request: Request, email_id: int):
    ...
    conn = get_db()
    row = conn.execute(
        "SELECT e.*, c.user_id FROM campaign_emails e JOIN campaigns c ON e.campaign_id = c.campaign_id WHERE e.id = ?",
        (email_id,),
    ).fetchone()
    ...
    profile_row = conn.execute(
        "SELECT cv_text FROM cv_profiles WHERE user_id = ? ORDER BY id DESC LIMIT 1",
        (user_id,),
    ).fetchone()
    cv_text = (
        profile_row["cv_text"]
        if profile_row and profile_row["cv_text"]
        else "No CV content provided."
    )
    ...
    try:
        ...
        conn.execute(
            "UPDATE campaign_emails SET interview_prep = ? WHERE id = ?",
            (prep_html, email_id),
        )
        conn.commit()
    except Exception as e:
...
def view_interview_prep(request: Request, email_id: int):
    ...
    conn = get_db()
    row = conn.execute(
        "SELECT * FROM campaign_emails WHERE id = ?", (email_id,)
    ).fetchone()
    conn.close()
    if not row:
        return RedirectResponse("/dashboard", status_code=303)
    ...
    return HTMLResponse(
        _build_dashboard_shell(
            None, user_id, content_html, "Interview Prep", "interview-prep"
        )
    )
...
def battle_station_page(request: Request):
    ...
    campaigns = []
    running_count = paused_count = completed_count = failed_count = total_sent = (
        total_responses
    ) = 0
    ...
    try:
        ...
        failed_count = sum(
            1 for c in campaigns if c.get("status") in ("failed", "error")
        )
        ...
        logger.error(
            f"Error in /battle-station query for user {user_id}: {e}", exc_info=True
        )
        user = {}
    finally:
        conn.close()
    ...
    return HTMLResponse(
        _build_dashboard_shell(
            user, user_id, content, "&#x2694;&#xFE0F; Battle Station", "battle-station"
        )
    )
