...
async def create_squad(request: Request):
    ...
    async with db.pool.acquire() as conn:
        ...
        user = await conn.fetchrow("SELECT squad_id FROM users WHERE user_id = $1", user_id)
        if user["squad_id"]:
        ...
        async with conn.transaction():
            ...
            """, squad_id, referral_code, user_id)
...
async def join_squad_via_invite(request: Request, referral_code: str):
    ...
    async with db.pool.acquire() as conn:
        owner = await conn.fetchrow("SELECT user_id, squad_id FROM users WHERE referral_code = $1", referral_code)
        if not owner or not owner["squad_id"]:
            return RedirectResponse(url="/register?error=invalid_invite")
            
        squad = await conn.fetchrow("SELECT * FROM job_squads WHERE squad_id = $1", owner["squad_id"])
...
async def process_pending_squad(conn, user_id: str, squad_id: str):
    ...
    await conn.execute("UPDATE users SET squad_id = $1 WHERE user_id = $2", squad_id, user_id)
    ...
    if squad["member_count"] == 3:
        ...
        await conn.execute("""
            ...
            WHERE squad_id = $2
        """, thirty_days, squad_id)
        
        await conn.execute("UPDATE job_squads SET is_complete = TRUE WHERE squad_id = $1", squad_id)
        logger.info(f"[VIRAL LOOP] Squad {squad_id} completed! 3 users upgraded to Premium.")
