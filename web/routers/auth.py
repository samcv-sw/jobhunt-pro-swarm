...
def _check_rate_limit(store: dict, ip: str, window_seconds: int = 3600, max_count: int = 10) -> bool:
    now = time.time()
    try:
        conn = get_db()
        ...
        if row:
            ...
            if db_count >= max_count:
                conn.close()
                return False
            conn.execute("REPLACE INTO system_config (key, value) VALUES (?, ?)", (db_key, f"{db_time}:{int(db_count)+1}"))
            conn.commit(); conn.close()
...
def google_callback(code: str = None, error: str = None):
    if error or not code:
        return HTMLResponse("<html><body><h3>OAuth Error</h3><p>Google authentication failed.</p><a href='/login'>Try again</a></body></html>")
    ...
    if not resp.ok: return HTMLResponse("<html><body><h3>OAuth Error</h3><p>Failed to exchange token.</p></body></html>")
    ...
    if not email: return HTMLResponse("<html><body><h3>OAuth Error</h3><p>Email not provided by Google.</p></body></html>")
    ...
    if not user:
        ...
        conn.execute(
            """INSERT INTO users (user_id, email, password_hash, name, api_key, oauth_provider, oauth_access_token, oauth_refresh_token, oauth_expires_at)
               VALUES (?, ?, ?, ?, ?, 'google', ?, ?, ?)""",
            (user_id, email, pwd_hash, name, generate_api_key(), access_token, refresh_token, expires_at)
        )
    else:
        user_id = user["user_id"]
        if refresh_token:
            conn.execute("UPDATE users SET oauth_provider='google', oauth_access_token=?, oauth_refresh_token=?, oauth_expires_at=? WHERE user_id=?", (access_token, refresh_token, expires_at, user_id))
        else:
            conn.execute("UPDATE users SET oauth_provider='google', oauth_access_token=?, oauth_expires_at=? WHERE user_id=?", (access_token, expires_at, user_id))
    conn.commit(); conn.close()
    
    response = RedirectResponse("/user-dashboard", status_code=303)
    response.set_cookie("user_id", get_session_serializer().dumps(user_id), httponly=True, samesite="lax", max_age=86400*30)
    return response

@router.get("/auth/microsoft/login")
def microsoft_login():
    scopes = ["openid", "email", "profile", "offline_access", "https://graph.microsoft.com/Mail.Send", "https://graph.microsoft.com/User.Read"]
    import urllib.parse
    auth_url = (
...
def microsoft_callback(code: str = None, error: str = None):
    ...
    if not resp.ok: return HTMLResponse("<html><body><h3>OAuth Error</h3><p>Token exchange failed.</p></body></html>")
    ...
    if not email: return HTMLResponse("<html><body><h3>OAuth Error</h3><p>Email address not provided.</p></body></html>")
    ...
    if not user:
        ...
        user_id = user["user_id"]
        if refresh_token:
            conn.execute("UPDATE users SET oauth_provider='microsoft', oauth_access_token=?, oauth_refresh_token=?, oauth_expires_at=? WHERE user_id=?", (access_token, refresh_token, expires_at, user_id))
        else:
            conn.execute("UPDATE users SET oauth_provider='microsoft', oauth_access_token=?, oauth_expires_at=? WHERE user_id=?", (access_token, expires_at, user_id))
    conn.commit(); conn.close()
...
def login_page(request: Request, plan: str = ""):
    ...
    return templates.TemplateResponse(request, "login_v2.html", {"selected_plan": selected_plan, "plan": plan, "VERSION": config.VERSION})
...
def login(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    if not _check_login_rate_limit(client_ip):
        return templates.TemplateResponse(request, "login_v2.html", {"error": "Too many login attempts."})
    ...
    if lockout:
        try:
            if time.time() - float(lockout["value"]) < 1800:
                conn.close(); return templates.TemplateResponse(request, "login_v2.html", {"error": "Account locked. Try again in 30 minutes."})
        except: pass
        
    user = conn.execute("SELECT * FROM users WHERE email = ?", (email,)).fetchone()
    if user and (user.get("is_active") == 0 or user.get("banned") == 1):
        conn.close(); return templates.TemplateResponse(request, "login_v2.html", {"error": "Account disabled."})
    ...
    if not user or not verify_password(password, user["password_hash"]):
        try:
            ...
            conn.execute("REPLACE INTO system_config (key, value) VALUES (?, ?)", (fail_key, str(fails)))
            if fails >= 5: conn.execute("REPLACE INTO system_config (key, value) VALUES (?, ?)", (account_key, str(time.time())))
            conn.commit()
        except: pass
        conn.close(); return templates.TemplateResponse(request, "login_v2.html", {"error": "Invalid credentials"})
    ...
    response = RedirectResponse("/user-dashboard", status_code=303)
    response.set_cookie("user_id", get_session_serializer().dumps(user["user_id"]), httponly=True, samesite="lax", max_age=86400*30)
    return response
