...
try:
    ...
    class ASGItoWSGI:
        ...
        async def _h(self, e, s):
            ...
            sc = {'type':'http','asgi':{'version':'3.0'},'http_version':'1.1','method':m,'headers':h,'path':p,'query_string':q.encode(),'root_path':e.get('SCRIPT_NAME',''),'scheme':e.get('wsgi.url_scheme','http'),'server':sv}
            st = [200]; rh = [[]]; bp = []; sd = [False]
            async def r(): return {'type':'http.request','body':b,'more_body':False}
            async def se(msg):
                if msg['type']=='http.response.start': st[0]=msg['status']; rh[0]=[(k.decode(),v.decode()) for k,v in msg.get('headers',[])]; sd[0]=True
                elif msg['type']=='http.response.body': bp.append(msg.get('body',b''))
            await self.asgi_app(sc,r,se)
            m = {200:'OK',301:'Moved',302:'Found',303:'See Other',400:'Bad Request',401:'Unauthorized',403:'Forbidden',404:'Not Found',422:'Unprocessable',500:'Server Error'}
            s(str(st[0])+' '+m.get(st[0],'OK'),rh[0])
            return [b''.join(bp)]
