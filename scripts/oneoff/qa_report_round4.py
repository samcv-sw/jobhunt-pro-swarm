...
def main():
    ...
    logger.debug("=" * 60)
    logger.debug("  QA REPORT ROUND 4 -- SECONDARY PAGES")
    logger.debug("=" * 60)
    logger.debug(f"  Total:  {report['summary']['total']}")
    logger.debug(f"  [PASS]: {report['summary']['passed']}")
    logger.debug(f"  [FAIL]: {report['summary']['failed']}")
    logger.debug()
    ...
    if failed:
        ...
        for f in failed:
            ...
            logger.debug(f"  [X] {f['file']}{partial_tag}")
            for issue in f.get("issues", []):
                logger.debug(f"      -> {issue}")
    else:
        logger.debug("  [OK] ALL FILES PASSED!")

    logger.debug()
    logger.debug(f"Full report -> qa_report_round4.json")
    logger.debug("=" * 60)
