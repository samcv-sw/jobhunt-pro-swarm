...
def main():
    ...
    if not ar_po.exists():
        print("Initializing Arabic locale...")
        run_cmd(f'pybabel init -i "{POT_FILE}" -d "{LOCALES_DIR}" -l ar')
    else:
        print("Updating Arabic locale...")
    ...
    if not en_po.exists():
        print("Initializing English locale...")
        run_cmd(f'pybabel init -i "{POT_FILE}" -d "{LOCALES_DIR}" -l en')
    else:
        print("Updating English locale...")
    ...
    print("Done! Translations are ready.")
