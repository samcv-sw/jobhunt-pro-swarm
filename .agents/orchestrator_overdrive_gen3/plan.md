...
|---|------|-------|-------------|--------|
| 1 | Backend Performance & Sync | Optimize event loop (<50ms latency), DB worker reconnect & retry. | None | PLANNED |
| 2 | Frontend UI/UX & RTL Polish | Audit physical CSS properties, check Next.js build, verify glassmorphism. | None | PLANNED |
| 3 | Scraper Stealth & Parsing | Audit anti-bot bypass & ensure structured dictionary returns (`title`, `url`). | None | PLANNED |
| 4 | Security & Auth | Audit endpoints for JWT verification & rate-limiting. | None | PLANNED |
| 5 | Full Test Suite & Audit | Run entire test suite and forensic integrity audits. | M1, M2, M3, M4 | PLANNED |
