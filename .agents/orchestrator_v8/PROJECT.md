# PROJECT: JobHunt Pro Max Overdrive Audit and Harden

## Architecture
- **Frontend**: Next.js app in `/frontend` using Tailwind CSS, supporting RTL/LTR via Logical CSS and Cairo/Tajawal fonts.
...
|---|------|-------|-------------|--------|
| 1 | R1. Frontend UI/UX & RTL Polish | Ensure all CSS/layouts use logical properties, next/fonts for Cairo/Tajawal, dir="auto" on inputs. | None | IN_PROGRESS |
| 2 | R2. Backend Concurrency & Sync | Celery tasks concurrency, asyncpg retry mechanism in sync_worker.py. | None | IN_PROGRESS |
| 3 | R3. Scraper Stealth Hardening | Upgrade stealth_ingest.py to bypass anti-bot, use profiles/proxies, return structured data. | None | IN_PROGRESS |
| 4 | R4. Security Hardening | JWT verification on `/api/v1/*` endpoints, auth routes, session checks. | None | IN_PROGRESS |
| 5 | R5. CI/CD Pipeline & E2E Validation | Setup production.yml workflow, run E2E test suite and pass all tests. | M1, M2, M3, M4 | IN_PROGRESS |
