"""
NODRIVER HEADLESS RUNNER — patches headless=True for cron execution
"""
...
async def scrape_bayt(browser, title, country_code, country_name):
    jobs = []
    slug = title.replace(' ', '-')
    ...
    try:
        ...
        for href, title_html in h2s:
            ...
            if len(title_t) > 5 and 'jobs in' not in title_t.lower():
                jobs.append({
                    'title': title_t, 'company': 'Unknown',
                    'url': f"https://www.bayt.com{href}",
                    'source': 'bayt', 'location': country_name
                })
        print(f"  Bayt [{title[:30]}/{country_code}]: {len(jobs)} jobs", flush=True)
    except Exception as e:
        print(f"  Bayt [{title}/{country_code}]: ERROR - {str(e)[:60]}", flush=True)
    return jobs

async def main():
    t_start = datetime.now()
    print(f">>> NODRIVER v3 BAYT COLLECTOR (HEADLESS) - {t_start.strftime('%H:%M:%S')}", flush=True)
    ...
    for title in TITLES:
        for country_code, country_name in COUNTRIES:
            total_queries += 1
            jobs = await scrape_bayt(browser, title, country_code, country_name)
